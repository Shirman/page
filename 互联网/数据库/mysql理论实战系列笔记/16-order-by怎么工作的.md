...
- 全字段排序
    ...
    上面7个步骤是通常排序的普遍执行流程，作者称作全字段排序。

- rowid排序

    在通常排序流程中，由于排序的行数据长度受到限制，在步骤3只会将name和主键id放入sort_buffer中，在步骤7时按name排序取钱1000行外，还需要回表查询city、name、age字段值，多了1000次的回表扫描。（但在排序内存不大的情况下，mysql就会换成为rowid的排序算法）
...
- 什么情况下可以不执行排序
    ...
    **比如city和name，我们可以考虑idx_city_name的联合索引，在city等值查询时，name就是天然有序的，执行时不会去做排序，且取到1000行时，就会结束返回数据。**
...
- 内存排序与文件排序
    ...
    临时文件排序：会根据排序数量大小生成一定数量的文件，最后归并排序。性能会很差。

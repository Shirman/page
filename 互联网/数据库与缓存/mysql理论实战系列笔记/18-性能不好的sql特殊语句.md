...
- 条件字段函数查询不支持索引树**搜索**，会对索引树进行**遍历**操作；遍历时，优先选择树最小的索引树
...
- 条件字段类型转换
    ...
    select * from t where order_no=123456;

    order_no定义varcha(32)，explain时，发现走了全表扫描，这个是因为order是字符串类型，但输入的是整型，需要做类型转换。

    在mysql中，select "10">9，会返回1，说明mysql在字符串与整型比较的时候，使用的规则是将字符串转换成数字。

    所以实际上上面的语句会被优化器转换成：
    select * from t where CAST(order_no as signed int) = 123456;

    上面我们提到条件字段函数会放弃索引树搜索。

- explain

    每次在业务逻辑中调整或使用新的sql语句时，先explain下是个不错的习惯。


- 优化器偷懒

    比如where id + 1>1000, 和where id>1000 -1，后一种写法效率更高，前一种写法会影响使用索引树搜索，且优化器还很大可能不会帮忙优化
...
图1 t\_modified索引示意图如果你的SQL语句条件用的是where t\_modified='2018-7-1’的话，引擎就会按照上面绿色箭头的路线，快速定位到 t\_modified='2018-7-1’需要的结果。
...
图2 explain 结果key="t\_modified"表示的是，使用了t\_modified这个索引；我在测试表数据中插入了10万行数据，rows=100335，说明这条语句扫描了整个索引的所有值；Extra字段的Using index，表示的是使用了覆盖索引。
...
图6 explain 结果这个语句里trade\_detail 表成了驱动表，但是explain结果的第二行显示，这次的查询操作用上了被驱动表tradelog里的索引(tradeid)，扫描行数是1。
...
<font size=2 color=grey>[阅读原文](https://time.geekbang.org/column/article/74059)</font>

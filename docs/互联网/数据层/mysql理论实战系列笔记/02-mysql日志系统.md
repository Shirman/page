...
- [笔记](#笔记)
- [原文](#原文)
    - [重要的日志模块：redo log](#重要的日志模块redo-log)
...
- redo log 与 binlog
  ...
  redo是是InnoDB引擎的且是物理的，binlog是mysql sercer层的且是逻辑的；现在由于redo只用于InnoDB引擎，但我们又可能使用其他引擎，所以必须要有binlog。

  redo log 可以理解为临时日志，内存级别日志，尚未写入磁盘

  binlog 永久日志，已确定日志，硬盘级别日志
...
- 两阶段提交 
  ...
  保证数据库的一致性，必须要保证2份日志一致，使用的2阶段式提交；和事务的概念有点相像，不是成功就是失败，不能让中间环节出现，也就是一个成功，一个失败。叫两阶段提交。

  prepare 和 commit 两个阶段，保证redo log与binlog的一致性，避免后期从binlog重放日志事务时，与原库相比缺失或多出事务操作


## 原文

> 原文：建议底部去阅读原文，还有丁奇老师的问答回复更是精华之处。
...
而粉板和账本配合的整个过程，其实就是MySQL里经常说到的`WAL`技术，WAL的全称是`Write-Ahead Logging`，它的关键点就是先写日志，再写磁盘，也就是先写粉板，等不忙的时候再写账本。
...
update语句执行流程你可能注意到了，最后三步看上去有点“绕”，将redo log的写入拆成了两个步骤：prepare和commit，这就是"两阶段提交"。
...
<font size=2 color=grey>[阅读原文](https://time.geekbang.org/column/article/68633)</font>

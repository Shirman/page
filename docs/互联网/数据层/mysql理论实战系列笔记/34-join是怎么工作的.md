...
- 索引嵌套循环join    
    ...
    - 取出被驱动表中满足条件的行，与数据行R组成一行，作为结果集的一部分
    - 重复执行以上步骤，直到驱动表循环结束
    
    在上面这个过程中，**驱动表走的全表扫描，被驱动表走的树搜索**
  
- 简单嵌套循环join

  没有使用到驱动表或被驱动表中的索引，需要全表扫驱动表，每次从驱动表取一行去被驱动表匹配的时候，也需要对被驱动表进行全表扫描，总共需要扫描（驱动表行数*被驱动表行数）次

  所以简单嵌套循环join不是好的join方式
...
- 分块嵌套循环join
  ...
  分块嵌套循环与简单嵌套循环的时间复杂度是一样的，但由于被驱动表数据匹配判断时，都是在内存join_buffer中操作，速度会比简单嵌套循环好很多。
...
图4 不使用索引字段join的 explain结果可以看到，在这个过程中，对表t1和t2都做了一次全表扫描，因此总的扫描行数是1100。由于join\_buffer是以无序数组的方式组织的，因此对表t2中的每一行，都要做100次判断，总共需要在内存中做的判断次数是：100\*1000=10万次。
...
图5 Block Nested-Loop Join -- 两段图中的步骤4和5，表示清空join\_buffer再复用。
...
我们前面的例子是没有加条件的。如果我在语句的where条件加上 t2.id

```
select * from t1 straight_join t2 on (t1.b=t2.b) where t2.id
```
...
```
select t1.b,t2.* from t1 straight_join t2 on (t1.b=t2.b) where t2.id
```
...
<font size=2 color=grey>[阅读原文](https://time.geekbang.org/column/article/79700)</font>
